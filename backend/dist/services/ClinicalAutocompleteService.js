...
class ClinicalAutocompleteService {
    ...
    async searchDiagnosisCodes(searchTerm, options = {}) {
        ...
        const { limit = this.DEFAULT_LIMIT, offset = 0, category, includeInactive = false, userFavoritesFirst = true, userId } = options;
    ...
    async searchMedications(searchTerm, options = {}) {
        ...
        const startTime = Date.now();
        const { limit = this.DEFAULT_LIMIT, offset = 0, category, includeInactive = false, userId } = options;
        let whereClause = "WHERE is_active = true";
        const params = [];
        let paramCount = 1;
        if (category) {
      ...
      search_keywords && ARRAY[$${paramCount + 4}]
        ...
        params.push(`%${searchTerm}%`, `%${searchTerm}%`, `%${searchTerm}%`, searchTerm, searchTerm.toLowerCase());
        paramCount += 5;
        const query = `
      SELECT 
        m.*,
    ...
    `;
        ...
        const result = await database_1.pool.query(query, params);
        if (userId) {
            await this.logSearchAnalytics(userId, searchTerm, 'MEDICATION', result.rows.length, Date.now() - startTime);
        }
        return result.rows.map(row => ({
            id: row.id,
    ...
    `;
        params.push(`${searchTerm}%`, `${searchTerm}%`, searchTerm, userId || null, limit, offset);
        const result = await database_1.pool.query(query, params);
        if (userId) {
            await this.logSearchAnalytics(userId, searchTerm, 'LAB_TEST', result.rows.length, Date.now() - startTime);
        }
        return result.rows.map(row => ({
            id: row.id,
            code: row.test_code,
    ...
    `;
        ...
        const result = await database_1.pool.query(query, params);
        if (userId) {
            await this.logSearchAnalytics(userId, searchTerm, 'PROCEDURE', result.rows.length, Date.now() - startTime);
        }
        return result.rows.map(row => ({
            id: row.id,
