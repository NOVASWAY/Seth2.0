...
async function testProductionFeatures() {
    ...
    try {
        ...
        const incrementalResult = await backupService.createIncrementalBackup(new Date(Date.now() - 24 * 60 * 60 * 1000) // 24 hours ago
        );
        ...
        const analyticsAggregation = await mongodb_1.mongoModels.Analytics.aggregate([
        ...
        const performanceMetrics = await mongodb_1.mongoModels.Analytics.find({
        ...
        await mongodb_1.mongoModels.Analytics.countDocuments();
        const mongoQueryTime = Date.now() - startTime;
        console.log(`⚡ MongoDB query time: ${mongoQueryTime}ms`);
        const pgStartTime = Date.now();
        await DatabaseService_1.databaseService.query('SELECT COUNT(*) FROM patients');
        ...
        const bulkPromises = [];
        for (let i = 0; i < 50; i++) {
            bulkPromises.push(mongodb_1.mongoModels.Analytics.create({
        ...
        await mongodb_1.mongoModels.Analytics.find({ event_type: 'benchmark_test' }).limit(100);
        const queryTime = Date.now() - queryStartTime;
        console.log(`⚡ Query performance: ${queryTime}ms`);
        // Cleanup benchmark data
        await mongodb_1.mongoModels.Analytics.deleteMany({ event_type: 'benchmark_test' });
