...
async function testAdvancedFeatures() {
    ...
    try {
        ...
        const migrationStatus = await MigrationService_1.MigrationService.getMigrationStatus();
        ...
        if (migrationStatus.postgresql.patients > 0) {
            ...
            const patientMigrationResult = await MigrationService_1.MigrationService.migratePatientData();
        ...
        await AnalyticsService_1.AnalyticsService.trackEvent('test_event', {
            testData: 'This is a test event',
            timestamp: new Date()
        });
        await AnalyticsService_1.AnalyticsService.trackEvent('patient_visit', {
            patientId: 'test-patient-123',
            visitType: 'consultation',
            duration: 30
        });
        await AnalyticsService_1.AnalyticsService.trackEvent('system_event', {
        ...
        const systemMetrics = await AnalyticsService_1.AnalyticsService.getSystemMetrics();
        ...
        const eventTypeStats = await mongodb_1.mongoModels.Analytics.aggregate([
            { $group: { _id: '$event_type', count: { $sum: 1 } } },
            { $sort: { count: -1 } }
        ]);
        console.log('📊 Event type statistics:', eventTypeStats);
        // Test time-based queries
        const recentEvents = await mongodb_1.mongoModels.Analytics.find({
            timestamp: { $gte: new Date(Date.now() - 24 * 60 * 60 * 1000) }
        }).sort({ timestamp: -1 }).limit(5);
        console.log('📅 Recent events (last 24h):', recentEvents.length);
        // Test clinical data queries
        const clinicalDataStats = await mongodb_1.mongoModels.ClinicalData.aggregate([
        ...
        const healthStatus = await DatabaseService_1.databaseService.getStatus();
        ...
        const promises = [];
        for (let i = 0; i < 10; i++) {
            promises.push(AnalyticsService_1.AnalyticsService.trackEvent('performance_test', {
                testId: i,
                timestamp: new Date(),
                data: `Test data ${i}`
            }));
        ...
        await mongodb_1.mongoModels.Analytics.deleteMany({
