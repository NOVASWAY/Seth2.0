...
router.post("/", (0, auth_1.authorize)([types_1.UserRole.ADMIN, types_1.UserRole.CLINICAL_OFFICER]), [
    ...
    (0, express_validator_1.body)("primary_diagnosis_code").notEmpty().withMessage("Primary diagnosis code is required"),
    (0, express_validator_1.body)("primary_diagnosis_description").notEmpty().withMessage("Primary diagnosis description is required"),
    (0, express_validator_1.body)("claim_amount").isNumeric().withMessage("Claim amount must be a number"),
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
...
router.put("/:id", (0, auth_1.authorize)([types_1.UserRole.ADMIN, types_1.UserRole.CLAIMS_MANAGER, types_1.UserRole.CLINICAL_OFFICER, types_1.UserRole.RECEPTIONIST]), [
    ...
    try {
        ...
        paramCount++;
        params.push(id);
        const result = await database_1.pool.query(`UPDATE sha_claims 
...
router.delete("/:id", (0, auth_1.authorize)([types_1.UserRole.ADMIN]), async (req, res) => {
    try {
        const { id } = req.params;
        const result = await database_1.pool.query(`DELETE FROM sha_claims WHERE id = $1 RETURNING *`, [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: "SHA claim not found"
