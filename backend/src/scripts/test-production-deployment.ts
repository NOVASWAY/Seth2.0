...
async function testProductionDeployment() {
  ...
  try {
    ...
    const pgResult = await databaseService.query('SELECT COUNT(*) as count FROM patients')
    ...
    const mongoCount = await mongoModels.Analytics.countDocuments()
    ...
    try {
      await mongoModels.ClinicalData.find({ invalidField: { $invalidOperator: 'test' } })
    } catch (error) {
    ...
    const finalHealth = await databaseService.getStatus()
    ...
    await Promise.all([
      databaseService.query('SELECT COUNT(*) FROM patients'),
      mongoModels.Analytics.countDocuments(),
      mongoModels.ClinicalData.countDocuments()
    ])
