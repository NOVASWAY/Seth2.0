  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      if (visitId) {
        ...
        visitsParams.push(visitId as string)
      }
      ...
      if (visitId) {
        ...
        encountersParams.push(visitId as string)
      }
      ...
      const encountersResult = await pool.query(encountersQuery, encountersParams)
      ...
      if (visitId) {
        ...
        prescriptionsParams.push(visitId as string)
      }
      ...
      const prescriptionsResult = await pool.query(prescriptionsQuery, prescriptionsParams)
      ...
      if (visitId) {
        ...
        labRequestsParams.push(visitId as string)
      }
      ...
      const labRequestsResult = await pool.query(labRequestsQuery, labRequestsParams)
      ...
      if (visitId) {
        ...
        claimsParams.push(visitId as string)
      }

      const claimsResult = await pool.query(claimsQuery, claimsParams)
      ...
      encounters.forEach(encounter => {
        if (encounter.treatments && Array.isArray(encounter.treatments)) {
          encounter.treatments.forEach((treatment: any) => {
            if (treatment.cost) {
      ...
      labRequests.forEach(labRequest => {
        if (labRequest.test_items && Array.isArray(labRequest.test_items)) {
          labRequest.test_items.forEach((item: any) => {
            if (item.cost) {
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const visitResult = await pool.query(
        `SELECT v.*, u.username as provider_name
         FROM visits v
         LEFT JOIN users u ON v.provider_id = u.id
      ...
      if (encounterId) {
        ...
        encounterParams.push(encounterId as string)
      }
      ...
      const encounterResult = await pool.query(encounterQuery, encounterParams)
      ...
      const invoiceData = {
        ...
        services: encounters.flatMap(encounter => 
          encounter.services.map((service: any) => ({
