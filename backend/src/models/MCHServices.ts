...
export interface PatientMCHService {
  ...
  serviceDate: Date
  providerId: string
  serviceDetails?: any // JSONB for flexible data
  findings?: string
...
export class MCHServicesModel {
  // Get all MCH services
  static async getServices(): Promise<MCHService[]> {
    const query = `
  ...
  static async getServicesByCategory(category: string): Promise<MCHService[]> {
    const query = `
      SELECT id, name, service_code as "serviceCode", category, description,
             target_population as "targetPopulation", frequency,
             is_active as "isActive", created_at as "createdAt", updated_at as "updatedAt"
  ...
  static async getPatientMCHServices(patientId: string): Promise<PatientMCHService[]> {
    const query = `
  ...
  static async getPatientMCHServicesByCategory(patientId: string, category: string): Promise<PatientMCHService[]> {
    const query = `
      SELECT pms.id, pms.patient_id as "patientId", pms.visit_id as "visitId",
             pms.service_id as "serviceId", pms.service_date as "serviceDate",
             pms.provider_id as "providerId", pms.service_details as "serviceDetails",
  ...
  static async createPatientMCHService(data: CreatePatientMCHServiceData): Promise<PatientMCHService> {
    const query = `
  ...
  static async updatePatientMCHService(id: string, data: Partial<CreatePatientMCHServiceData>): Promise<PatientMCHService | null> {
    const fields = []
    const values = []
    let paramCount = 1
  ...
  static async getMCHServiceStats(): Promise<any> {
    const query = `
      SELECT 
        ms.category,
  ...
  static async getUpcomingMCHAppointments(days: number = 7): Promise<any[]> {
    const query = `
      SELECT 
  ...
  static async deletePatientMCHService(id: string): Promise<boolean> {
